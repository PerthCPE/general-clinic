...
func advanceActiveQueues(rng *rand.Rand) {
	...
	for i := 0; i < countToAdvance && i < len(activeQueues); i++ {
		...
		case "กำลังตรวจ":
			// ขั้นที่ 3: ตรวจเสร็จ -> สุ่ม ~30% ไปห้องหัตถการ หรือ 70% ไปชำระเงิน
			if rng.Intn(10) < 3 {
				...
				nextNote = "ตรวจเสร็จสิ้น รอสรุปค่ารักษาพยาบาล"
			}

		case "รอทำหัตถการ":
			// ขั้นที่ 4: ทำหัตถการเสร็จ -> ส่งชำระเงิน
			nextStatus = "รอชำระเงิน"
			nextDept = "ห้องการเงิน (แคชเชียร์)"
			nextNote = "ทำหัตถการเรียบร้อย รอชำระค่ารักษาพยาบาล"

		case "รอชำระเงิน":
			// ขั้นที่ 5: ชำระเงินเสร็จ -> ส่งไปห้องจ่ายยา
			nextStatus = "รอรับยา"
			nextDept = "ห้องจ่ายยาและเภสัชกรรม"
			nextNote = "ชำระเงินแล้ว รอจัดยาและรับคำแนะนำการใช้ยา"

		case "รอรับยา":
			// ขั้นที่ 6: รับยาเสร็จสิ้น -> ปิดคิว
			nextStatus = "เสร็จสิ้น"
			nextDept = "ห้องจ่ายยาและเภสัชกรรม"
			nextNote = "รับยาและเสร็จสิ้นขั้นตอนการรักษาเรียบร้อย"
